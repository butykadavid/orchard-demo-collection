...
class Game {
  ...
  getCurrentScore() {
    ...
    const validations = this.rounds.reduce(
      (acc, round) => acc + round.validator.membersWithValueCount(),
      0
    );
...
class Round {
  ...
  setFirstDigit() {
    if (this.editable) {
      let newDigit = (parseInt(this.code[0]) + 1) % 5;
  ...
  setSecondDigit() {
    if (this.editable) {
      let newDigit = (parseInt(this.code[1]) + 1) % 5;
      if (newDigit === 0) newDigit = 5;
  ...
  setThirdDigit() {
    if (this.editable) {
      let newDigit = (parseInt(this.code[2]) + 1) % 5;
      if (newDigit === 0) newDigit = 5;
...
class Validator {
  ...
  membersWithValueCount() {
    const values = [this.a, this.b, this.c, this.d, this.e, this.f].filter(
      (v) => v !== undefined
    );
